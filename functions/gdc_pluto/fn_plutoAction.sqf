...
// Boucle sur tous les groupes sous le commandement de PLUTO
{
	...
	_targets = _targets select {(_unit knowsAbout _x) >= 1}; // Ne lancer des actions spéciales que si la cible est suffisament connue -> y a-t-il des cas où le knowsAbout est < à 1.6 ?
	...
	if (_count > 0) then {
		...
		switch (_group getVariable ["PLUTO_ORDER","DEFAULT"]) do {
			case "QRF": {[_group,_targets] call int_fnc_plutoDoQRF;};
			case "ARTY": {[_group,_targets] call int_fnc_plutoDoArty;};
			case "IGNORE";
			case "DEFAULT";
			default {};
		};
