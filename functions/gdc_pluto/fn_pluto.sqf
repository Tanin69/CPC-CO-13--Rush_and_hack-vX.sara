...
call int_fnc_plutoHearing;

// Lancement de Pluto
[_sides] spawn {
	params ["_sides"];
	waitUntil {time > 5};
	if (gdc_plutoDebug) then {systemChat "Pluton se réveille (version locale tanin)";};
	private _boucle = 0;
	while {gdc_plutoRun} do {
		...
		{
			private _return = [_x] call int_fnc_plutoAnalize;
			_return call int_fnc_plutoAction;
		} forEach _sides;
		sleep 10;
	};
};