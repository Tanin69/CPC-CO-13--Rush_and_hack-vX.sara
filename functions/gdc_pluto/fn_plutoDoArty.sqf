...
// Ne lancer une frappe que si la dernière frappe date de plus de x minutes (x étant défini par plutoArtyTimeout)
if ((time - (_group getVariable ["PLUTO_LASTORDER",0])) > (_group getVariable ["PLUTO_ARTYTIMEOUT",gdc_plutoArtyTimeout])) then {
	...
	*/
	...
	_range = _group getVariable ["PLUTO_ARTYRANGE",gdc_plutoArtyRange]; // Eventuel range custom
